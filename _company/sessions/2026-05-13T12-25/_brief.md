# 📋 작업 브리프

**원 명령:** [자율 사이클 — 2026-05-13] 1인 기업 24시간 운영 중. 회사 목표·각 에이전트의 개인 목표(_agents/{id}/goal.md)·최근 의사결정·메모리를 검토해서 지금 가장 가치 있는 단일 작업 1개를 결정하고, 적절한 1~2명 에이전트에게 분배해서 실행하세요. 같은 산출물을 반복하지 마세요 — 메모리에 비슷한 항목이 24시간 내에 있으면 다른 각도로 진전시키세요.

## 요약
이번 작업은 소셜 미디어 채널인 인스타그램에서 게시 시간과 캡션을 최적화하여 콘텐츠 홍보 전략을 개선하는 것입니다.

## 분배
- **📷 Instagram**: 1. 현재 인스타그램 채널의 게시 시간과 캡션을 분석하고, 일관된 스타일과 트렌드에 맞는 최적화 전략을 제안하세요.
2. 인스타그램 홍보 전략을 개선하기 위해 소셜 미디어 에이전트와 협력하여 실행 가능한 단계별 프로젝트 관리를 수행하세요.
