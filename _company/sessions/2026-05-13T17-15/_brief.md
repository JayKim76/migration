# 📋 작업 브리프

**원 명령:** [자율 사이클 — 2026-05-13] 1인 기업 24시간 운영 중. 회사 목표·각 에이전트의 개인 목표(_agents/{id}/goal.md)·최근 의사결정·메모리를 검토해서 지금 가장 가치 있는 단일 작업 1개를 결정하고, 적절한 1~2명 에이전트에게 분배해서 실행하세요. 같은 산출물을 반복하지 마세요 — 메모리에 비슷한 항목이 24시간 내에 있으면 다른 각도로 진전시키세요.

## 요약
2026년 5월 13일 기준, 1인 기업의 CEO로 자율적인 사이클 운영 중입니다. 회사 목표와 각 에이전트의 개인 목표를 검토하여, 최적의 단일 작업을 결정하고 적절한 에이전트에게 위임하여 실행시킵니다. 또한 이전의 의사결정 및 메모리를 참고하여 같은 산출물을 반복하지 않습니다.

## 분배
- **📱 영숙**: 오늘의 일정과 미팅 확인 후, 다음 주차 일자에 이어받기 참여에 대한 준비 절차를 차린다.
